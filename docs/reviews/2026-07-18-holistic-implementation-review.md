...
- **Surfaced, not folded — parole `_extract_bias_flags`.** Parole carries the *same* private-parser shape as the retail two, but sits outside F1's documented retail scope (it predates the recipe; the review classified it "not a gap" because it bounds via `sanitize_for_prompt`). Its test is now exact (F2); migrating its parser to the shared helper is a **candidate follow-up**, not done here — it touches convergence logic, so per fold-in policy it is surfaced for a conscious decision rather than silently changed.
