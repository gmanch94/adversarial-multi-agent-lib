...
flowchart TB
    ...
    Pyt(["pytest + pytest-asyncio<br/>766 lib + 112 sibling tests · mypy strict · ruff clean"]):::user
...
| Tests | **766 library tests + 112 sibling tests** (post Tier 2.1c-2 ship 2026-05-18): `pytest -k unit` (pure logic, no API) + `pytest -k integration` (fake agents via DI) covering all 6 domains + durable subpackage + 4 production sibling deployments | Caller writes their own tests against their workflows |
