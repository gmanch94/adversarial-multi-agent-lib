...
Updated through the 2026-07-24 lifesciences advanced-therapies (CGT/ATMP) 5th-segment MVP-8 ship (27/27 catalog + 8 CGT = 35 workflows). Prior cycles: durable POC + 5 production siblings (durable_postgres · _k8s · _otel · cipher_gcp_kms · cipher_aws_kms) + Tier-2.1 multi-tenant; 2026-05-16 healthcare ([`security-audits/2026-05-16-healthcare-sweep.md`](security-audits/2026-05-16-healthcare-sweep.md)); 2026-05-14 industrial + PC; 2026-05-12 initial.
...
flowchart TB
    ...
    subgraph Process["Caller Python process - asyncio event loop"]
        ...
        subgraph Lib["adv-multi-agent library"]
            ...
            subgraph LifeSciDom["lifesciences/ (35 · 27 catalog + 8 CGT/ATMP)"]
                LSD["substantial_equivalence_510k (veto) · assay_performance_claim (veto)<br/>combination_product_pmoa · device_reportability (veto)<br/>field_action_classification (veto) · promotional_off_label_review (veto)<br/>design_control_traceability · nutrition_health_claim<br/>gxp_data_integrity · computer_system_validation · stability_shelf_life<br/>batch_release_deviation (veto) · cmo_qualification · udi_labeling<br/>clinical_protocol_design (veto) · pharmacovigilance_signal (veto)<br/>CGT/ATMP: hctp_classification (veto) · cgt_potency_assay (veto)<br/>cgt_comparability (veto) · donor_eligibility (veto) · cgt_durability_claim (veto)<br/>rmat_designation · vector_safety · cgt_lot_release_spec"]:::lib
            end
            subgraph CorePkg["core/"]
                ...
                SkillsReg["SkillRegistry · MCP server<br/>(288 bundled templates · SKILLS_DOMAIN)"]:::lib
            end
...
flowchart TB
    ...
    Pyt(["pytest + pytest-asyncio<br/>2059 lib + 207 sibling tests · mypy strict · ruff clean"]):::user
    ...
    subgraph Host["Host machine — Windows / macOS / Linux"]
        ...
        subgraph Pkg["adv-multi-agent (pip install -e .)"]
            ...
            SkillsDir["bundled templates (wheel)<br/>15 research + 6 parole + 34 retail + 29 pc + 32 industrial + 32 healthcare + 140 lifesciences = 288"]:::lib
        end
    end
...
| Skill files | Bundled in wheel (288 templates across 7 domains); local override via `Config(skills_dir=...)` | Same |
| Network | Live API calls — costs real money per run | Same — there is no mock mode |
| Tests | **2059 library tests + 207 sibling tests**: `pytest -k unit` (pure logic, no API) + `pytest -k integration` (fake agents via DI) covering all 7 domains + durable subpackage + 5 production sibling deployments | Caller writes their own tests against their workflows |
...
- **No audit log of model I/O in core.** stdout + the user's chosen logging is all there is; regulated-context users add their own at the caller layer. (The durable subpackage ships a pluggable `AuditSink` seam — Tier 3.1, D-AUDIT-1..8.)
