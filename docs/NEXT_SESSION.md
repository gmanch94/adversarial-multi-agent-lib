...
## ⚠ Open — CI is RED on `main` (pre-existing, needs your call; NOT caused by G8)
- Discovered while confirming the G8 push's CI status. Last **code** push `c768994` (D-A11-7, 2026-07-25) **FAILED CI at the ruff step** (`I001` unsorted imports · `RUF100` unused `# noqa: E402` · `S110` try-except-pass in `examples/production/_shared/tenant_env.py` + siblings). Three `[skip ci]` docs commits since meant no CI ran, so it sat invisible; mypy/pytest never ran on the 3.11–3.13 matrix after 2026-07-23 (ruff blocked them). The G8 push was `[skip ci]`-HEAD so it too was matrix-skipped.
- **Root cause:** ruff is unpinned — `"ruff>=0.4,<1.0"` in `pyproject.toml:45`. CI installs whatever version is latest at install time; local ruff **0.15.12** passes the same tree clean (`ruff check .` exit 0). Version drift = non-deterministic CI. Config is minimal (`[tool.ruff]` = `line-length` only; no `[tool.ruff.lint]` select/exclude).
- **Recommended fix (not applied — your call, touches build config + unknown post-ruff matrix state):** pin `ruff==0.15.12` (the locally-verified-good version) in the `[dev]` extra so `local == CI` is reproducible, then push the pin as a **code** commit (no `[skip ci]`) so the full matrix finally runs — that both greens ruff and reveals whether mypy/pytest pass on 3.11–3.13 (unknown since 2026-07-23). Alternatives: fix the 3 lint classes in `examples/production/` (leaves the drift), or exclude `examples/` from lint (reduces coverage — weaker). Pinning is the durable root-cause fix.
- To re-check before deciding: `gh run list --limit 5`.
