...
- **Full `needs_postgres` run:** 277 passed. The 9 non-audit failures are **pre-existing, not mine** — stale `test_store`/`test_tenant_isolation` that assume a non-super `daemon` (throwaway image makes it superuser) + `delete()` missing the Tier-2.1b `tenant_id` kwarg + 4 `cipher_gcp_kms` cloud-cred errors. Tracked as a separate needs_postgres test-hygiene follow-up; DO NOT attribute to Tier 3.1.
