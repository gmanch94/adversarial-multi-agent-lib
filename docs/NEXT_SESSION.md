...
Last updated: 2026-07-25 (PM) — **CGT ship-audit spine findings BOTH fixed.** Tree clean after the H-1 commit, **nothing pushed** (direct-to-main pre-launch; user pushes). Both reproduced-live spine defects closed: **C-1 FIXED** (D-A11-5, `fca3c5a`) and **H-1 FIXED** (D-A11-6, this session) — the `max_chars=6000` request-text guillotine retired across all 67 workflows via one shared `sanitize_request_text` helper (control-char pass + 60k field-count backstop; per-field 1500 caps unchanged) + author-time guard **G7** + a behavioural regression. Independent reviewer SHIP-WITH-FOLDIN, 2 stale-comment fold-ins closed pre-commit. **Remaining backlog (none blocking):** M-1, M-2, M-3, L-1..L-8. Prior: CGT/ATMP 5th segment SHIPPED (D-LIFESCI-7/8, 8 workflows); needs_postgres hygiene; Tier 3.1 audit log (D-AUDIT-1..8).
...
- **(this session)** — **H-1 FIX (D-A11-6), spine/convention.** Shared `sanitize_request_text(request)` in `core/_internal.py` replaces `sanitize_for_prompt(request.to_prompt_text(), max_chars=6000)` at all 67 domain call sites (control-char/NFC pass + `_MAX_REQUEST_PROMPT_CHARS=60_000` field-count backstop; per-field `_MAX_FIELD_CHARS=1500` unchanged). Bytes-level codemod preserved retail CRLF. Guard **G7** (no workflow calls `.to_prompt_text()` directly) + behavioural regression `test_hctp_classification.py::TestH1FullRequestReachesExecutor` (verified fails at HEAD's 6000 cap, passes fixed) + `test_internal.py::TestSanitizeRequestText`. Gate: ruff · mypy 120 · **1942 tests**. Per-field visible markers deferred = **L-IND-5** stays a known gap (distinct, smaller class — every field still represented).

**OPEN — backlog (none blocking; in order):**
1. **M-1 (MEDIUM):** flag headers line-anchored inside reviewer-criteria prose (8 modules + `lotrelease_review.md`) — a restated rubric can shadow real findings (same root class as C-1, but fail-CLOSED on the gate; damages finding integrity). Re-wrap so no header sits at line start outside the emission block. Prompt change → deliberate review.
2. **L-1:** add **G8** to `test_workflow_conventions.py` — assert no gate uses a bare `not any(...)` over flag values (retires the D-A11-1 class at author-time; the durable win). *(G7 is now the H-1 request-text guard.)*
3. **M-2 / M-3:** example+spine hygiene — `core/wiki.py` `context_for_round` scopes on round only (cross-case wiki bleed; donor PHI); predictable `/tmp` example workspaces (symlink pre-creation). L-2..L-8 backlog (see report table).

**Things NOT to do next:** don't push without the user (nothing has been pushed). Don't reintroduce the per-call-site `sanitize_for_prompt(request.to_prompt_text(), max_chars=N)` cap — route request text through `sanitize_request_text` (G7 enforces; H-1/D-A11-6). Don't "fix" H-1 per-module or per-field-budget (`6000//n` starves CMC narratives) — the convention fix landed. Don't reintroduce occurrence-selection in the parser (C-1/D-A11-5 — selection is unsafe first-wins AND last-wins; union/first-non-empty is the fix). Don't reuse D-LIFESCI-1..8 (all taken; next CGT Phase-2 batch = D-LIFESCI-9+). Don't take G7 for L-1 (it's G8 now).
...
- **Then an independent CODE review (verdict FIX-BEFORE-SHIP) caught two more:** HIGH — terminal `run_completed`/`run_failed` events had NO working outbox path (resume refuses terminal runs, the sweep only reports) → added `DurableWorkflow.reemit_audit(token)` (recovers any status, idempotent; **operator-invoked — daemon auto-wiring is a follow-up**). MEDIUM — round events used the mutable `cp.updated_at`, so under outbox lag every un-emitted decision's timestamp collapsed to resume time → each entry now carries an immutable `at` at append. Plus 3 LOWs (AuditEvent↔DB bound parity, intentional-`default=str` note, 0008 idempotency claim). All fixed; 33 audit tests + 15 sibling.
- **Advisor final check flagged a false-closure — now RESOLVED by a live-DB run.** The DB layer had never executed (no Docker/DSN); I added `tests/test_audit_sink_pg.py` (needs_postgres). **2026-07-24: brought up Docker + a throwaway `postgres:16-alpine`, ran it — 3/3 pass.** Layers 1 (chain append, `hashtext::bigint` advisory lock) + 2 (RLS-reject-cross-tenant + append-only-grant-reject UPDATE/DELETE, against a provisioned NOSUPERUSER role — a superuser bypasses RLS, so the RLS test creates its own non-super role) are now integration-verified. SECURITY_MODEL A9 promoted to unqualified **CLOSED**. Layer 3 (S3 Object-Lock anchor WRITER) stays a cloud integration verified at deploy (like the KMS ciphers); its all-anchors verify LOGIC is unit-tested.
- **Full `needs_postgres` run:** 277 passed. 9 pre-existing non-audit failures were tracked separately (not Tier 3.1) — **now FIXED (2026-07-24, see next section).**

## 2026-07-24 — needs_postgres test-hygiene (9 pre-existing failures fixed)

Full `durable_postgres` needs_postgres suite now **167 passed, 0 failed** (was 9 failing) against a throwaway `postgres:16-alpine`. Two classes, all test-only (no `src/` change):

- **Test-drift (5) — Tier 2.1b `tenant_id` never backfilled into these tests:** `delete()` calls missing the required `tenant_id=` kwarg (`test_delete_idempotent`→renamed `test_delete_then_redelete_raises_run_not_found`, `test_store_rejects_bad_run_id_before_touching_db`); raw `INSERT INTO checkpoints` missing the `tenant_id` NOT-NULL column so it hit NotNull before the `run_id` CHECK it means to test (`test_run_id_charset_constraint_at_db_layer` + 2 `test_integrity_tag_roundtrip` legacy-payload inserts). Also: `delete()` is **not idempotent** post-2.1b (Q6: raises `RunNotFound` on 0 rows) — the renamed test asserts the current contract.
- **RLS tests (4) — superuser bypasses RLS, so they proved nothing against the throwaway image's superuser role.** New `conftest.nonsuper_pool` fixture provisions a **NOSUPERUSER, non-owner** role with DML grants (same mechanism `test_audit_sink_pg` already uses). Each RLS test now pairs a **same-role positive control** (same-tenant write succeeds) with the cross-tenant negative — an RLS WITH-CHECK reject and a missing grant both surface as 42501, so the positive control is what proves the reject is RLS, not a grant gap (advisor gate). `test_rls_quarantine_insert_rejects_cross_tenant` had NO positive control — added.
- Touched: `tests/conftest.py` (+`nonsuper_pool`), `test_store.py`, `test_tenant_isolation.py`, `test_integrity_tag_roundtrip.py`. Ruff clean. Sibling test-function count unchanged (rewrote bodies, added 1 fixture).
- **Env note — Docker Desktop 4.79.0 was crash-looping** on its Model-Runner "Inference manager" (malformed `unix://C:/...` socket, un-removable stale `dockerInference`). Fixed by setting **`EnableDockerAI: false`** in `%APPDATA%\Docker\settings-store.json` — left disabled (re-enabling reintroduces the crash until Docker Desktop is upgraded/reset).
