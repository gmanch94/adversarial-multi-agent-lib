...
- **Advisor final check flagged a false-closure (fixed):** the DB layer (0008 DDL, grants, FORCE-RLS, advisory lock, CHECKs) + WORM anchor were **never executed** — no Docker/DSN in the ship env, so the `needs_postgres` tests skipped and the 15 sibling tests are pure Python. SECURITY_MODEL A9 downgraded from unqualified CLOSED to "library seam + chain logic CLOSED; DB-layer + anchor execution PENDING a live-Postgres run" (now agrees with gaps §3.1). Added `tests/test_audit_sink_pg.py` (needs_postgres — chain append, dedup, RLS cross-tenant reject) as the durable gate; it **collects+skips clean here but is UNRUN**. **Next-session first task: bring up the sibling `docker compose` Postgres, set `POSTGRES_DSN`, run `test_audit_sink_pg.py` + one live anchor+walk to complete layer-2/3 verification and promote A9 to unqualified CLOSED.**
