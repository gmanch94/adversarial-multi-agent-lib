...
| Skills | **212 bundled templates** (15 research + 6 parole + 34 retail + 29 pc + 32 industrial + 32 healthcare + 64 lifesciences) inside the wheel; local override via `Config(skills_dir=...)` | Versioned, signed, distributed via the registry |
...
| **`lifesciences/` (27 of 27-workflow catalog · COMPLETE)** | Submissions: `SubstantialEquivalence510kWorkflow` (veto) · `AssayPerformanceClaimWorkflow` (veto) · `CombinationProductPMOAWorkflow` · `BiosimilarComparabilityWorkflow` (veto) · `BioequivalenceWorkflow` (veto). Post-market Safety: `DeviceReportabilityWorkflow` (veto) · `FieldActionClassificationWorkflow` (veto) · `PharmacovigilanceSignalWorkflow` (veto) · `PostMarketClinicalFollowupWorkflow` · `CCDSLabelChangeWorkflow` (veto) · `REMSDesignWorkflow`. Promo/Labeling/MedInfo: `PromotionalOffLabelReviewWorkflow` (veto) · `UDILabelingWorkflow` · `NutritionHealthClaimWorkflow` · `MedicalInformationResponseWorkflow` (veto). Quality/GxP/Mfg: `DesignControlTraceabilityWorkflow` · `GxPDataIntegrityWorkflow` · `ComputerSystemValidationWorkflow` · `BatchReleaseDeviationWorkflow` (veto) · `CMOQualificationWorkflow` · `StabilityShelfLifeWorkflow` · `SterilityAssuranceWorkflow` (veto) · `ColdChainExcursionWorkflow` (veto) · `SerializationDSCSAWorkflow` · `PremarketCybersecurityWorkflow`. Clinical/Access: `ClinicalProtocolDesignWorkflow` (veto) · `HEORDossierWorkflow`. 14 veto / 27 workflows. 108 skill templates. Manufacturer-regulatory boundary distinct from healthcare (D-LIFESCI-1); catalog complete (batches A+B shipped) — [design doc](superpowers/specs/2026-07-19-lifesciences-domain-design.md). | [`lifesciences/workflows/*.py`](../src/adv_multi_agent/lifesciences/workflows/) |
