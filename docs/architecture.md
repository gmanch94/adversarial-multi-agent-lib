...
| Skills | **288 bundled templates** (15 research + 6 parole + 34 retail + 29 pc + 32 industrial + 32 healthcare + 140 lifesciences) inside the wheel; local override via `Config(skills_dir=...)` | Versioned, signed, distributed via the registry |
...
flowchart LR
    ...
    subgraph Lib["adv-multi-agent library — caller's Python process"]
        ...
        subgraph Research["research/"]
            Workflows[Workflows<br/>review_loop · idea_discovery<br/>rebuttal · manuscript_assurance]
        ...
        subgraph LifeSciDomain["lifesciences/ (35 · 27 catalog + 8 CGT/ATMP)"]
            LSWf["Submissions: substantial_equivalence_510k (veto)<br/>assay_performance_claim (veto) · combination_product_pmoa<br/><br/>Post-market Safety: device_reportability (veto)<br/>field_action_classification (veto) · pharmacovigilance_signal (veto)<br/><br/>Promo/Labeling: promotional_off_label_review (veto)<br/>udi_labeling · nutrition_health_claim<br/><br/>Quality/GxP: design_control_traceability · gxp_data_integrity<br/>computer_system_validation · batch_release_deviation (veto)<br/>cmo_qualification · stability_shelf_life<br/><br/>Clinical: clinical_protocol_design (veto)<br/><br/>Adv. Therapies (CGT/ATMP): hctp_classification (veto) · cgt_potency_assay (veto)<br/>cgt_comparability (veto) · donor_eligibility (veto) · cgt_durability_claim (veto)<br/>rmat_designation · vector_safety · cgt_lot_release_spec"]
        end
        Workflows --> Agents
        Workflows --> Stores
        Assurance --> Agents
    ...
    Researcher --> Caller --> HW
    Researcher --> Caller --> LSWf
    Agents --> Anthropic
    Agents --> Gemini
    Agents --> OpenAI
    Stores --> Ledger
...
| **`lifesciences/` (35 workflows · 27 catalog + 8 CGT/ATMP)** | Submissions: `SubstantialEquivalence510kWorkflow` (veto) · `AssayPerformanceClaimWorkflow` (veto) · `CombinationProductPMOAWorkflow` · `BiosimilarComparabilityWorkflow` (veto) · `BioequivalenceWorkflow` (veto). Post-market Safety: `DeviceReportabilityWorkflow` (veto) · `FieldActionClassificationWorkflow` (veto) · `PharmacovigilanceSignalWorkflow` (veto) · `PostMarketClinicalFollowupWorkflow` · `CCDSLabelChangeWorkflow` (veto) · `REMSDesignWorkflow`. Promo/Labeling/MedInfo: `PromotionalOffLabelReviewWorkflow` (veto) · `UDILabelingWorkflow` · `NutritionHealthClaimWorkflow` · `MedicalInformationResponseWorkflow` (veto). Quality/GxP/Mfg: `DesignControlTraceabilityWorkflow` · `GxPDataIntegrityWorkflow` · `ComputerSystemValidationWorkflow` · `BatchReleaseDeviationWorkflow` (veto) · `CMOQualificationWorkflow` · `StabilityShelfLifeWorkflow` · `SterilityAssuranceWorkflow` (veto) · `ColdChainExcursionWorkflow` (veto) · `SerializationDSCSAWorkflow` · `PremarketCybersecurityWorkflow`. Clinical/Access: `ClinicalProtocolDesignWorkflow` (veto) · `HEORDossierWorkflow`. Advanced Therapies (CGT/ATMP, 5th segment): `HCTPClassificationWorkflow` (veto) · `CGTPotencyAssayWorkflow` (veto) · `CGTComparabilityWorkflow` (veto) · `DonorEligibilityWorkflow` (veto) · `CGTDurabilityClaimWorkflow` (veto) · `RMATDesignationWorkflow` · `VectorSafetyWorkflow` · `CGTLotReleaseSpecWorkflow`. 19 veto / 35 workflows. 140 skill templates. Manufacturer-regulatory boundary distinct from healthcare (D-LIFESCI-1); 27/27 catalog complete (batches A+B) + 8 CGT MVP shipped (D-LIFESCI-7/8, 4 CGT Phase-2 locked) — [catalog design doc](superpowers/specs/2026-07-19-lifesciences-domain-design.md) · [CGT segment design doc](superpowers/specs/2026-07-24-lifesciences-cgt-segment-design.md). | [`lifesciences/workflows/*.py`](../src/adv_multi_agent/lifesciences/workflows/) |
...
| **`_internal`** | Shared utilities: JSON parser, atomic write, redaction, score coercion, path sandboxing, prompt sanitization, `sanitize_request_text` (D-A11-6 — the request-text boundary at all 67 workflow call sites; per-field 1500 cap in `to_prompt_text` + `_MAX_REQUEST_PROMPT_CHARS=60_000` field-count backstop; retired the old 6000 post-concat cap), `extract_flags` (line-anchored; D-A11-5 unions every header occurrence; H-IND-1 hyphen-tolerant sibling-stop), `extract_veto_directive` (M-PC-1 line-anchored; D-A11-5 first-non-empty; M2/L5 + H-IND-1), `truncate_flag_display` (L-PC-5), `_is_sibling_header_lhs` (shared sibling-stop helper). Colon-suffixed flag and `REVIEWER VETO:` headers live only in the reviewer emission block, never in criteria prose (D-A11-7, guard G9). Used by every flag-gated + veto-using workflow across all 7 domains. | [`core/_internal.py`](../src/adv_multi_agent/core/_internal.py) |
...
| A9 | Core library emits no structured audit log of model inputs/outputs. The durable subpackage now ships a pluggable `AuditSink` seam (Tier 3.1, D-AUDIT-1..8) for caller-owned audit capture on durable runs; non-durable core remains caller-side. | **Seam shipped (durable); core caller-side** | First user needing core (non-durable) audit capture |
...
| A11 | ~~No test coverage yet.~~ **2059 library tests + 207 sibling tests passing** across research + parole + retail + pc + industrial + healthcare + lifesciences + durable + durable_postgres sibling + durable_postgres_k8s sibling + durable_postgres_otel sibling + cipher_gcp_kms sibling + cipher_aws_kms sibling + shared; mypy strict; ruff clean. | ✅ Resolved — Phase 2 complete + sustained through 7 domain ships + durable POC + 5 sibling deployments | — |
...
The full phase status is in **[`build-plan.md`](build-plan.md)**; Phases 1–6 complete. Work since (durable subpackage, 5 production siblings, multi-tenant, and the retail / pc / industrial / healthcare / lifesciences domains) is tracked in [`decisions.md`](decisions.md) + [`NEXT_SESSION.md`](NEXT_SESSION.md), not as build-plan phases.
