...
flowchart LR
    ...
    subgraph Lib["adv-multi-agent library — caller's Python process"]
        ...
        subgraph LifeSciDomain["lifesciences/ (16 of 27 catalog · MVP-8 + Phase-2 batch A)"]
...
| **`lifesciences/` (16 of 27-workflow catalog · MVP-8 + Phase-2 batch A)** | Submissions: `SubstantialEquivalence510kWorkflow` (veto) · `AssayPerformanceClaimWorkflow` (veto) · `CombinationProductPMOAWorkflow`. Post-market Safety: `DeviceReportabilityWorkflow` (veto) · `FieldActionClassificationWorkflow` (veto) · `PharmacovigilanceSignalWorkflow` (veto). Promo/Labeling: `PromotionalOffLabelReviewWorkflow` (veto) · `UDILabelingWorkflow` · `NutritionHealthClaimWorkflow`. Quality/GxP: `DesignControlTraceabilityWorkflow` · `GxPDataIntegrityWorkflow` · `ComputerSystemValidationWorkflow` · `BatchReleaseDeviationWorkflow` (veto) · `CMOQualificationWorkflow` · `StabilityShelfLifeWorkflow`. Clinical: `ClinicalProtocolDesignWorkflow` (veto). 8 veto / 16 workflows. 64 skill templates. Manufacturer-regulatory boundary distinct from healthcare (D-LIFESCI-1); 11 Phase-2 batch-B designs remaining in [design doc](superpowers/specs/2026-07-19-lifesciences-domain-design.md). | [`lifesciences/workflows/*.py`](../src/adv_multi_agent/lifesciences/workflows/) |
