...
> **STATUS: FIXED** (2026-07-25, after this report was filed) — retired repo-wide as **D-A11-7**, not only the CGT surface. Every colon-suffixed flag header was removed from reviewer-criteria PROSE across all 44 flag-gated workflows + their 36 mirror `*_review.md` templates (100 exact-case occurrences via a bytes-level codemod that preserved retail CRLF, plus 4 retail lowercase-slash-format scoring lines — `zero A / B / CAPACITY flags:` — the exact-case pass missed); the colon-form now lives only in the emission block, so no restated rubric can shadow real findings. Guard **G9** (`tests/unit/test_workflow_conventions.py`, modules + templates, all prongs mutation-tested) fails any colon-header in criteria prose via two prongs matching both runtime matchers: a case-sensitive generic `[A-Z…] FLAGS:` (the `_is_section_header` sibling terminator — catches a NON-tuple header, fail-OPEN, worse than M-1) and each declared header case-insensitively + whitespace-flexibly (`_header_anchor_re` is `(?mi)`). Independent reviewer verdict SHIP-WITH-FOLDIN; the hardened guard then caught 4 live misses the first exact-case sweep left. **FOLLOW-UP LANDED (same session, user-confirmed):** the `REVIEWER VETO:` marker — the same terminator class on the higher-consequence halt marker, surfaced by the independent review — was then swept from prose across all 53 veto files (30 workflows + 23 templates, 106 occurrences); the emission `REVIEWER VETO: <verbatim…>` line and the `extract_veto_directive` code marker are unchanged. G9 gained a `REVIEWER VETO:` prong (mutation-tested), so the whole section-header-echo class (FLAGS + VETO) is retired from criteria prose. The original finding is preserved below unedited.
...
> **STATUS: FIXED** (2026-07-27) — resolved at the caller layer, not the spine, and locked as **D-A11-9**. All 44 determination examples (35 lifesciences + 8 healthcare + 1 parole) now set `workspace_dir=tempfile.mkdtemp(prefix="<name>-")`, so each run gets a fresh empty wiki and no prior case can bleed in. The 5 cwd-literal ones (4 healthcare `.healthcare_workspace` + `parole_assessment`) were surfaced by the independent pre-commit review, not the original `/tmp` sweep — same M-2 class, no `/tmp/` spelling. `ResearchWiki` / `context_for_round` gained a scope-note docstring making the round-only (not run-scoped) filtering explicit — the wiki is a documented cross-run shared store (research accumulation relies on it), so run-isolation is the per-case caller's job, not new spine behaviour. Guard **G10** (`test_workflow_conventions.py`, mutation-tested) fails any domain example that hardcodes `/tmp/`. Gate: ruff · mypy 120 · 2346 tests. The original finding is preserved below unedited.
...
> **STATUS: FIXED** (2026-07-27) — closed by the same D-A11-9 change: `tempfile.mkdtemp()` creates the workspace atomically with mode 0700 and an unguessable name, so the symlink-preplant race is gone — there is no predictable directory for an attacker to pre-create. Applied across all 44 determination examples via the M-2 sweep, not the 8 CGT ones only; the world-writable `/tmp` target itself was the 39 `/tmp` examples, and the 5 cwd-literal ones (healthcare `.healthcare_workspace` + `parole`) are M-2-only but took the same `mkdtemp` fix. G10's two prongs keep both the `/tmp` spelling and the fixed-literal `workspace_dir` shape from regressing; an example that sets no `workspace_dir` (Config default `"."`) is a documented residual it does not cover. `core/config.py` `safe_resolve_path` is unchanged — the caller no longer hands it a predictable shared path; an `O_NOFOLLOW` hardening of `Config` stays an option if a fixed workspace is ever reintroduced. The original finding is preserved below unedited.
...
| L-2 **(DOWNGRADED)** | `accumulated[header].extend(...)` never resets — ~4.8 MB worst case in `WorkflowResult.metadata` at 50 rounds. Never re-injected into a prompt; metadata dedups. | `hctp:320` +7 | Cap accumulation, or dedup on write | Behaviour change · see resolution below |
...
| L-6 **(FIXED)** | Ledger writes silently swallowed when `config.max_claim_text_chars > 2000` — `ClaimLedger` is constructed without `max_claim_chars`, `add` raises, and `workflow.py:117-118` discards it. Empty audit trail, no warning. | `core/workflow.py:113-118`; `core/ledger.py:25,113` | Pass `max_claim_chars` through, or warn on the swallow | Shared spine · see resolution below |
...
### Backlog resolutions (post-audit)

> **L-6 — FIXED (2026-07-27, D-A11-10).** Root cause reproduced live against this tree before any edit: `Config(max_claim_text_chars=5000)` is valid (`1 ≤ x ≤ 1_000_000`, `config.py:190`) but `BaseWorkflow.__init__` built `ClaimLedger(config.ledger_path)` with the ledger's own hardcoded `_DEFAULT_MAX_CLAIM_CHARS = 2000` (`ledger.py:23`); `_register_claims` truncated each claim to the *Config* cap (5000), so a 2001–5000-char claim reached `ClaimLedger.add`, tripped `_bound`'s `raise ValueError`, and was discarded by the `except ValueError: continue` at `workflow.py:117-118` — a silent audit-trail gap. Repro: a 3000-char claim landed as `total=1` (short claim only), `warnings=[]`. **Fix, both halves:** (1) bind the ledger to the same value — `ClaimLedger(config.ledger_path, max_claim_chars=config.max_claim_text_chars)` — so a claim that clears the truncation is persistable; behaviour-neutral except the exact `config > 2000` bug case (under default config the 1000-char truncation was already the binding constraint, never the 2000 ledger cap). (2) the residual swallow (a caller-injected ledger with a smaller bound) now `warnings.warn(...)` — the message carries only the length + bound from `exc`, never claim text, so it is PHI-safe. Negative tests both halves in `tests/unit/test_workflow_register_claims.py::TestRegisterClaimsLengthBound` (the first fails pre-fix). The regression guard is the test, not a new CI gate: L-6 is a single shared-spine site (not a 59-file convention), so reverting the one-line wiring turns `test_claim_within_config_cap_survives_default_ledger` red. Gate: ruff 0.15.12 · mypy 120 · **2348 tests**.
>
> **L-2 — DOWNGRADED (2026-07-27), not fixed.** On reading the code the severity does not hold. The "4.8 MB in `WorkflowResult.metadata`" claim is already false: metadata dedups every flag list at read (`dict.fromkeys(accumulated[...])`, `hctp:347-353`), and the accumulator is never re-injected into any prompt. The one genuine residual is *intermittent* checklist `len()` inflation — `_build_hctp_checklist` counts the raw accumulated list (`len(mm_flags)`, `hctp:432/437/442`), so a flag whose **exact string** repeats across rounds is over-counted; but flags are free-text reviewer prose revised between rounds, so exact-string repeats are the exception. Weighed against a 59-file convention sweep (`accumulated[header].extend(current[header])` appears in 59 workflows) + a new shared helper + a new G-guard + 59 test files to re-verify, the trade is not justified for a sometimes-off-by-a-little count that is already deduped everywhere it matters. Revisit only if a real run surfaces an inflated count; the minimal fix if so is dedup-on-write (keeps metadata output identical, corrects the count).
...
**Backlog progress (2026-07-27):** L-6 LANDED (D-A11-10) · L-2 DOWNGRADED (self-mitigated — metadata dedups; see resolutions above). Remaining: L-3, L-4, L-5, L-7, L-8.
