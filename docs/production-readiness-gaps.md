...
**Phase-2 batch A (#9–16) ship-audit verdict:** FIX-BEFORE-SHIP → fixed, then clean. Independent reviewer audited all 8 new workflows on checks a–j (dim↔flag-header mapping, threshold parity in criteria+test, veto ordering, shared-helper inheritance, PHI-caveat matrix, boundary docstring, header hygiene, no-brand, input-shape, exact `== [...]` test assertions). All functional checks PASS. One MEDIUM — **D-LIFESCI-3 commercial-tool brand names** (ValGenesis/Argus in batch A; Windchill/Teamcenter/DOORS/TrackWise/PromoMats **pre-existing in the MVP-8 skeletons**) — genericized in `953f5ca`, tripwire denylist extended (base64). Separately, a mid-build registry probe surfaced a **core skill-registry parser bug**: the minimal YAML parser dropped block-sequence `inputs:` lists (parsed to `['']`), silently skipping ~60 templates across **5 domains** (parole/pc/industrial/healthcare/lifesciences-MVP8) — the MVP-8 "discoverable=32" claim was false. Fixed in `c1a7414` + a `test_registry.py` files-on-disk==discovered guard; all **212 templates now discoverable**.

**Universal PRODUCTION_GAPS (all 8 workflows — advisory decision-support only, never a regulatory submission):**

- **No live source-system integrations.** Every input is caller-pasted free text today, not a live query. Named generically per-workflow in each module's `PRODUCTION_GAPS:` docstring (no vendor product names, per D-LIFESCI-3): PLM + requirements-management tool + eQMS + ISO 14971 risk file (design-control); substantiation-dossier repository + nutrient DB + allergen-control plan (nutrition); 21 CFR 3 / OCP RFD-precedent DB (PMOA); LIMS + CLSI EP study data + clinical-study DB (assay); FDA 510(k) clearance DB + product-classification DB + eSTAR (substantial-equivalence); promotional-review DAM + approved-labeling repository (promo/MLR); complaint-handling QMS + FDA eMDR + EU EUDAMED (device-reportability); complaint/CAPA + FDA Recall Enterprise System + UDI/lot genealogy (field-action).
...
- **Ship-audit verdict (independent reviewer, checks a–i):** 0 CRITICAL / 0 HIGH / 1 MEDIUM / 3 LOW. All functional checks PASS on all 11 modules (dim↔flag-header mapping, threshold parity in criteria+test, veto audit-before-check ordering, shared-helper inheritance, PHI-caveat matrix, boundary docstrings, header hygiene, input bounding, exact `== [...]` assertions). The **MEDIUM** — a **D-LIFESCI-3 brand leak** ("Tyvek", a registered trademark, in the sterility example+test) — and two batch-B **LOW**s (ccds `first_draft` comment reworded to drop the `L-HEALTH-1` token so a `grep L-HEALTH-1` audit no longer false-hits; 6 veto per-field-cap asserts tightened `<=` → `==` for parity with the no-veto tests) were all fixed pre-push in `afb6be1`. One further **LOW — FIXED on confirmation** — `core/_internal.py` `extract_veto_directive` docstring had described the pre-H-IND-1 `isalpha()/isupper()` rule while the code calls `_is_sibling_header_lhs` (doc drift only; behavior was already correct). Surfaced (touches the shared veto-parser / convergence-logic area) and, on user go-ahead, the docstring was corrected to reference `_is_sibling_header_lhs` (H-IND-1: uppercase letters + spaces + hyphens). Docstring-only; parser tests unchanged and green.
