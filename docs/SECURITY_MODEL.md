...
| A reviewer that omits a FLAGS section silently satisfies that safety class (A11-M1) | **Closed 2026-07-23** — `missing_flag_headers()` + `BaseWorkflow._flag_classes_unresolved()` wired into all 59 flag-gated workflows: an unemitted section blocks convergence instead of reading as "clean". Guarded by `test_workflow_conventions.py` G5 (tuple ⊆ reviewer emission block) and `test_parser_hardening.py::TestA11M1MissingHeaderIsDistinguishable`. Verified end-to-end: dropping one class flips `converged` True→False |
| Reviewer formatting variance empties a flag class or drops a veto (A11-M1/M5) | **Closed 2026-07-23** — `_header_anchor_re` tolerates markdown emphasis, leading bullet/number, pre-colon whitespace, and case; both parsers take the LAST anchored match (A11-M4) so an earlier quoted `REVIEWER VETO: None` cannot suppress a real halt. 8 demonstrated deviations covered by `test_parser_hardening.py` |
| `sanitize_for_prompt` returned `max_chars + 14`, raising out of every `wiki.add_feedback` (A11-M3) | **Closed 2026-07-23** — marker is written inside the budget; return is `<= max_chars` for every cap. `Config.max_wiki_body_chars` now reaches `ResearchWiki`, and the four previously-unbounded size knobs are range-checked at construction (A11-L7) |
| `veto_reason` is rendered verbatim into operator-facing `WorkflowResult.output` (A11-M8) | **Closed 2026-07-23** — `extract_veto_directive` returns through `sanitize_for_prompt`, so control characters cannot reach the output surface; bounded by the caller's `max_chars` |
...
| Per-field `_MAX_FIELD_CHARS = 1500` truncation is silent (L-IND-5) | **Open** — reopened 2026-07-23 (depth review D4). A `cap_field` helper was added 2026-05-16 to emit a `UserWarning` on truncation and this row claimed the gap closed; a census found **zero callers** across all 63 workflows (including the 27 shipped after it), so the warning never fired anywhere. The helper was deleted rather than leave an unfollowed convention. Every `to_prompt_text` still uses a bare `value[:cap]` slice, then `sanitize_for_prompt(max_chars=6000)` post-concat — two-stage silent truncation, documented per-workflow as a PRODUCTION_GAP. Closing this requires adopting a warning helper across all 58 call sites, not adding one. |
