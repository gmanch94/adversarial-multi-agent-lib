...
| Retail request fields rendered into executor prompt | Prompt injection via free-text request | `*Request.to_prompt_text()` → `sanitize_for_prompt(..., max_chars=6000)` for every retail workflow (demand, labor, recall, loyalty, promo, supplier, replenishment) |
