---
name: claims_appeal_checklist
description: Approval checklist for claims appeal; routes to appropriate reviewer tier and documents decision
inputs: [flag_items]
---

# Claims Appeal Review — Coordinator Checklist

[OWNER: Appeals Coordinator / Medical Director]

## Required actions before issuing determination

- [ ] Confirm appeal received within plan timeline (72h urgent / 30 days standard ERISA)
- [ ] Verify coverage policy version effective at date of service
- [ ] Route first-level clinical appeals to medical director;
      second-level to external review organization
- [ ] Notify member of decision with appeal rights and timeline
- [ ] Document rationale citing specific coverage policy section + clinical evidence

## Flag resolution items

{flag_items}

## Sign-off

Medical Director (first-level clinical): ___________________________ Date: ________

External Review Organization (second-level): _______________________ Date: ________
