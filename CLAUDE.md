...
Package layout: `core/` (shared infra — agents, config, ledger, wiki, skills, MCP server, shared helpers `extract_flags` / `extract_veto_directive` / `truncate_flag_display` / `sanitize_for_prompt` / `_is_sibling_header_lhs`), `research/` (4 research workflows + assurance pipeline), `parole/` (1 workflow), `retail/` (8 workflows), `pc/` (7 workflows · Foundational + Specialty tracks), `industrial/` (8 MVP of 27-workflow catalog · 19 Phase-2 designs locked), `healthcare/` (8 MVP of 27-workflow catalog · 19 Phase-2 designs locked), `lifesciences/` (27 of 27-workflow catalog · MVP-8 + Phase-2 batch A + batch B · CATALOG COMPLETE). **63 workflows · 1681 library tests + 207 sibling tests · 256 skill templates · durable subpackage (incl. Tier 3.1 audit-log `AuditSink` seam) + 5 production siblings (durable_postgres, durable_postgres_k8s, durable_postgres_otel, cipher_gcp_kms, cipher_aws_kms).** The pattern is domain-agnostic; `core/` is the extension point.
...
- Don't add new workflows to `research/` or new domains beyond current scope without a decision entry. **Currently-shipped domains: research, parole, retail, pc, industrial, healthcare, lifesciences.** Phase-2 industrial + healthcare workflow promotion = fill-in against locked design, not new design — see [industrial design doc](docs/superpowers/specs/2026-05-14-industrial-domain-design.md) and [healthcare design doc](docs/superpowers/specs/2026-05-16-healthcare-domain-design.md). **Lifesciences is COMPLETE (27/27, batches A+B shipped)** — see [lifesciences design doc](docs/superpowers/specs/2026-07-19-lifesciences-domain-design.md).
