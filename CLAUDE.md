...
Package layout: `core/` (shared infra — agents, config, ledger, wiki, skills, MCP server, shared helpers `extract_flags` / `extract_veto_directive` / `truncate_flag_display` / `sanitize_for_prompt` / `sanitize_request_text` / `_is_sibling_header_lhs`), `research/` (4 research workflows + assurance pipeline), `parole/` (1 workflow), `retail/` (8 workflows), `pc/` (7 workflows · Foundational + Specialty tracks), `industrial/` (8 MVP of 27-workflow catalog · 19 Phase-2 designs locked), `healthcare/` (8 MVP of 27-workflow catalog · 19 Phase-2 designs locked), `lifesciences/` (35 workflows · 27-workflow catalog COMPLETE + advanced-therapies/CGT 5th-segment MVP-8 built · 4 CGT Phase-2 designs locked · D-LIFESCI-7/8). **71 workflows · 2348 library tests + 207 sibling tests · 288 skill templates · durable subpackage (incl. Tier 3.1 audit-log `AuditSink` seam) + 5 production siblings (durable_postgres, durable_postgres_k8s, durable_postgres_otel, cipher_gcp_kms, cipher_aws_kms).** The pattern is domain-agnostic; `core/` is the extension point.
...
- **Flag-header wiring (D-DEPTH-1 codifies the rule) — `_FLAG_HEADERS` must be load-bearing:** a multi-flag workflow declares `_FLAG_HEADERS: tuple[str, ...]` once and drives everything from it — `current` / `accumulated` via `{h: [] for h in _FLAG_HEADERS}`, extraction via `for header in _FLAG_HEADERS:`, the gate via `self._flag_classes_unresolved(review.critique, _FLAG_HEADERS, current.values())`, `accumulated` passed straight to the checklist builder, and `_format_flag_section(current)` taking the dict with a per-header `banner` map. **Never re-hardcode a header literal at the `extract_flags` call site** — a rename applied to the tuple but not the call site makes `extract_flags` return `[]` forever, permanently satisfying that flag class in the convergence gate (fail-OPEN, invisible to `any(...)`-shaped tests). Guarded by `tests/unit/test_workflow_conventions.py` G1+G2.
- **Convergence gate must use `_flag_classes_unresolved`, never bare `not any(...)` (D-A11-1):** `extract_flags` returns `[]` both for "reviewer wrote None detected" and "reviewer never emitted the section". Only the helper distinguishes them (via `missing_flag_headers`), so a dropped section blocks convergence instead of reading as clean. Guarded by **G8** (D-A11-8): the gate must route through the helper with LIVE args (headers arg is the `_FLAG_HEADERS` tuple, flags arg reads the flag values — a hollowed `_flag_classes_unresolved(critique, (), ())` is fail-OPEN, same drift class G2 catches for `extract_flags`), `run` carries no bare `not any(...)` over flag values (the legit short-circuit lives in the sibling `_format_flag_section`), and non-gated workflows are asserted to have grown no such gate. **Every `_FLAG_HEADERS` member must also appear in that module's reviewer-criteria emission block** (`End your review with…`) — the parser and the prompt are two halves of one contract, and G1/G2 cannot see the prompt half. Guarded by G5; lookup-key drift by G6.
- **Parser changes are shared-spine changes (D-A11-2):** `core/_internal.py` termination logic is biased toward over-collection on purpose — over-recognising a section header empties a flag class (fail-OPEN), under-recognising merely slurps a sibling (fail-safe). Keep that direction. Add a case to `tests/unit/test_parser_hardening.py` for any new deviation shape. **Never select one occurrence of a repeated header (D-A11-5):** `extract_flags` unions every occurrence, `extract_veto_directive` returns the first non-empty, and `_sections_for` bounds each slice at the next occurrence. First-wins loses to an earlier echo, last-wins to a later one — selection is unsafe in both directions, so do not re-introduce it.
- **Request-text boundary is `sanitize_request_text`, never a per-call-site cap (D-A11-6):** every workflow's `run()` renders its `*Request` via `request_text = sanitize_request_text(request)` (in `core/_internal.py`) — control-char/NFC pass + a `_MAX_REQUEST_PROMPT_CHARS=60_000` field-count backstop; the per-field `_MAX_FIELD_CHARS=1500` slice in `to_prompt_text` stays the real bound. **Never wrap request text in `sanitize_for_prompt(request.to_prompt_text(), max_chars=N)`** — the retired `N=6000` sat below `n_fields × 1500`, so a long request had its trailing fields hard-cut off the executor prompt, and since the reviewer only sees the executor draft that evidence left the whole adversarial loop (H-1). Guarded by G7 (no workflow calls `.to_prompt_text()` directly). Per-field visible truncation markers remain a separate known gap (L-IND-5 / D-DEPTH-3).
- **Colon-suffixed flag headers live only in the emission block, never in criteria PROSE (D-A11-7 / M-1):** a `<HEADER> FLAGS:` restated in a rubric body — a cross-ref (`Flag under X FLAGS:.`) or a scoring line (`zero X FLAGS: ready`) — is echoed by the reviewer and, once line-anchored in the critique, shadows the real findings/veto (`extract_flags` unions the placeholder, `extract_veto_directive` returns it). Refer to a section in prose WITHOUT its trailing colon (`flag under MOA-LINKAGE FLAGS`, `zero X FLAGS, then ready`). Guarded by **G9** (`test_workflow_conventions.py`, modules + `*_review.md` templates, mutation-tested) with two prongs matching the two runtime matchers: a case-sensitive generic `[A-Z…] FLAGS:` (the `_is_section_header` sibling terminator — catches a non-tuple header, fail-OPEN) and each declared header **case-insensitively** (the `_header_anchor_re` matcher is `(?mi)`, so `moa-linkage flags:` is a live match an exact-case check misses). The `REVIEWER VETO:` marker is the same terminator class and is retired from prose the same way (instruction `REVIEWER VETO: line` → `REVIEWER VETO line`, demo `"REVIEWER VETO: None"` → `"None"`); the emission `REVIEWER VETO: <verbatim…>` line and the `extract_veto_directive` code marker are unchanged, and G9's `REVIEWER VETO:` prong guards it.
- **Veto output (D-DEPTH-2):** every reviewer-veto workflow composes `{_VETO_BANNER}\n\nVETO DIRECTIVE: {veto_reason}\n\n--- Vetoed draft below ---\n\n{draft}\n\n---\n\n{_DISCLAIMER}`. The approver reads `WorkflowResult.output`; the directive must be there, not only in `metadata['veto_reason']`. Metadata scalars always go through `sanitize_for_prompt(..., max_chars=200)` — never bare `request.<field>` (L-HEALTH-2). Guarded by G3+G4.
...
- Don't add new workflows to `research/` or new domains beyond current scope without a decision entry. **Currently-shipped domains: research, parole, retail, pc, industrial, healthcare, lifesciences.** Phase-2 industrial + healthcare workflow promotion = fill-in against locked design, not new design — see [industrial design doc](docs/superpowers/specs/2026-05-14-industrial-domain-design.md) and [healthcare design doc](docs/superpowers/specs/2026-05-16-healthcare-domain-design.md). **Lifesciences 27-workflow catalog is COMPLETE (batches A+B shipped); a 5th segment — advanced therapies (cell & gene / ATMP), MVP-8 built + 4 Phase-2 locked (D-LIFESCI-7/8)** — see [lifesciences design doc](docs/superpowers/specs/2026-07-19-lifesciences-domain-design.md) and [CGT segment design doc](docs/superpowers/specs/2026-07-24-lifesciences-cgt-segment-design.md). Building the 4 CGT Phase-2 designs = fill-in, not new design.
