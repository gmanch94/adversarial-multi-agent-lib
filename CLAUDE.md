...
Package layout: `core/` (shared infra — agents, config, ledger, wiki, skills, MCP server, shared helpers `extract_flags` / `extract_veto_directive` / `truncate_flag_display` / `sanitize_for_prompt` / `_is_sibling_header_lhs`), `research/` (4 research workflows + assurance pipeline), `parole/` (1 workflow), `retail/` (8 workflows), `pc/` (7 workflows · Foundational + Specialty tracks), `industrial/` (8 MVP of 27-workflow catalog · 19 Phase-2 designs locked), `healthcare/` (8 MVP of 27-workflow catalog · 19 Phase-2 designs locked), `lifesciences/` (35 workflows · 27-workflow catalog COMPLETE + advanced-therapies/CGT 5th-segment MVP-8 built · 4 CGT Phase-2 designs locked · D-LIFESCI-7/8). **71 workflows · 1857 library tests + 207 sibling tests · 288 skill templates · durable subpackage (incl. Tier 3.1 audit-log `AuditSink` seam) + 5 production siblings (durable_postgres, durable_postgres_k8s, durable_postgres_otel, cipher_gcp_kms, cipher_aws_kms).** The pattern is domain-agnostic; `core/` is the extension point.
